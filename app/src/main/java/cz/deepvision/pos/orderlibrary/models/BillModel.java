...
public class BillModel {
            ...
            for (OrderItem item : getItems()) {
                if (item.getCategory().equals(category)) {
                    ...
                    if (SettingsManager.getInstance().isPrintProductCodes()) {
                        printItem = new String[]{item.getCount() + "x", "(" + item.getCode() + ")" + item.getName()};
                    } else {
                    ...
                    for (OrderItem addition : item.getSideDish()) {
                        String[] printAdditionItem = new String[]{"+" + String.valueOf(addition.getCount()) + "x", addition.getName()};
                        printList.add(printAdditionItem);
                    }

                    if (SettingsManager.getInstance().isPrintCoverBon()) {
                        for (OrderItem associated : item.getCover()) {
                            String[] printAssociatedItem = new String[]{"+" + String.valueOf(associated.getCount()) + "x", associated.getName()};
    ...
    public List<String[]> getCartPrintList() {
        EnumUtil.PriceType type = SettingsManager.getInstance().getDeliveryType();
        List<String[]> printList = new ArrayList<>();
        for (String category : BillManager.getInstance().getCurrentBill().getCategories()) {
            ...
            printProducts(type, printList, category);
        }
        return printList;
    }

    private void printProducts(EnumUtil.PriceType type, List<String[]> printList, String category) {
        ...
        for (OrderItem item : getItems()) {
            if (item.getCategory().equals(category)) {
                if (SettingsManager.getInstance().isSpeedloEnabled()) {
                    ...
                    String[] printItem = new String[]{String.valueOf(item.getCount()) + "x", item.getName() + "(" + String.format("%.2f", item.getPrice()) + BillManager.getInstance().getCurrency() + ")", String.format("%.2f", productSum) + " " + BillManager.getInstance().getCurrency()};
                    printList.add(printItem);

                    for (OrderItem addition : item.getSideDish()) {
                        double additionSum;
                        additionSum = addition.getCount() * item.getPrice();
                        String[] printAdditionItem = new String[]{"+" + String.valueOf(addition.getCount()) + "x", addition.getName() + "(" + String.format("%.2f", item.getPrice()) + " )" + BillManager.getInstance().getCurrency(), String.format("%.2f", additionSum) + BillManager.getInstance().getCurrency()};
                        printList.add(printAdditionItem);
                    }
                    ...
                    for (OrderItem associated : item.getCover()) {
                        ...
                        String[] printAssociatedItem = new String[]{"+" + String.valueOf(associated.getCount()) + "x", associated.getName() + "(" + String.format("%.2f", item.getPrice()) + " " + BillManager.getInstance().getCurrency() + ")", String.format("%.2f", associatedSum) + BillManager.getInstance().getCurrency()};
                        printList.add(printAssociatedItem);
                    }
                }
            }
